...
namespace OrderService.Infrastructure.Repositories
{
    public class MasterDataRepository : IMasterDataRepository
    {
        ...
        public async Task<List<CancellationPolicy>> GetActiveCancellationPoliciesAsync(DateTime asOfDate)
        {
            return await _dbContext.CancellationPolicies
                .AsNoTracking()
                .Where(c => c.IsActive && c.CreatedAt <= asOfDate)
                .ToListAsync();
        }
        ...
        public async Task<List<ReturnPolicy>> GetActiveReturnPoliciesAsync(DateTime asOfDate)
        {
            return await _dbContext.ReturnPolicies
                .AsNoTracking()
                .Where(r => r.IsActive && r.CreatedAt <= asOfDate)
                .ToListAsync();
        }
